...
class DLModel:
    ...
    def predict(self, X):
        ...
        for l in range(1,L):
            ...
            if (self.layers[l]._activation == "softmax" and self.layers[l]._activation == "softmax_trim"):
                is_softmax = True
        if (is_softmax == True):
            return np.where(Al == np.max(Al), 1, 0)
        else:  
    ...
    def confusion_matrix(self, X, Y):
        ...
        cf = self.confusion_matrix(prediction_index, Y_index)
        print(cf)
        return cf   
...
class DLLayer:
    def __init__(self, name, num_units, input_shape : tuple, activation="relu", W_initialization = "random", learning_rate = 1.2, optimization=None, random_scale = 0.01):
        ...
        if activation == "sigmoid":
            ...
            self.activation_backward=  self._trim_softmax_backwards
        else:
    ...
    def _softmax(self, Z):
        C = np.sum(np.exp(Z), axis=0)
        aL = np.exp(Z) / C
        return aL
    
    def _softmax_backwards(self, dZ):
        return dZ
    ...
    def _trim_softmax_backwards(self, dZ):
        return dZ
